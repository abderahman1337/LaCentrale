...
<div>
    <div class="flex items-center justify-between">
        <h2 class="lg:text-2xl text-xl font-bold">Les options <span class="text-primary">({{$options->total()}})</span></h2>
    ...
    <div class="mt-6">
        <form method="GET" action="{{route('admin.options.index')}}" class="w-full mx-auto mb-4">
            <div class="flex">
                <label for="search-input" class="mb-2 text-sm font-medium text-gray-900 sr-only dark:text-white">Equipement</label>
                <select class="rounded-l-lg border-gray-300 text-gray-900 bg-gray-50 text-sm focus:ring-0 w-32 lg:w-auto" name="equipment" id="equipment-select">
                    <option value="">Equipement</option>
                    @foreach ($equipments as $equipment)
                    <option value="{{$equipment->id}}" @selected(request('equipment')==$equipment->id)>{{$equipment->name}}</option>
                    @endforeach
                </select>
